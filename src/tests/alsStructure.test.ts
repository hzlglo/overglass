...
describe('ALS Structure Verification', () => {
  ...
  it('should have correct parameter counts per track', async () => {
    ...
    console.log(
      'T1 parameters:',
      t1Params.map((p) => p.parameterName),
    );
    ...
    console.log(
      'T6 parameters:',
      t6Params.map((p) => p.parameterName),
    );
  ...
  it('should have automation points for all parameters', async () => {
    ...
    for (const track of tracks) {
      ...
      for (const param of parameters) {
        const points = await db.automation.getAutomationPoints({ parameterId: param.id });
  ...
  it('should debug track loading process', async () => {
    ...
    devices.forEach((device, index) => {
      console.log(
        `Device ${index}: ${device.deviceName} (${device.trackCount} tracks, ${device.parameterCount} parameters)`,
      );
    });
    ...
    if (devices.length > 0) {
      ...
      tracks.forEach((track) => {
        console.log(
          `  Track ${track.trackNumber}: ${track.trackName} (${track.parameterCount} params, ${track.automationPointCount} points)`,
        );
      });
